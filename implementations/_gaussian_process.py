...
class GaussianProcess2:
    ...
    def Neglikelihood(self, theta):
        ...
        # Correlation length
        # it is beneficial to search for θ on a logarithmic scale to accelerate the convergence of the optimization
        # algorithm
        theta = 10**theta
...
def test_1d():
    ...
    # GP model training
    GP = GaussianProcess2(n_restarts=10, optimizer='L-BFGS-B')
...
if __name__ == '__main__':
    ...
    y_test = func_3D(X_test)

    # Train data
    X_train = (ub-lb)*lhs(3, samples=50) + lb
    y_train = func_3D(X_train).reshape(-1,1)

    # GP model training
    pipe = Pipeline([('scaler', MinMaxScaler()), ('GP', GaussianProcess2(n_restarts=40, optimizer='L-BFGS-B'))])
